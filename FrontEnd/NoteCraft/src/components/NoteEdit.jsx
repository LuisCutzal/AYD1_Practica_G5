...
const NoteEdit = ({ isOpen, onClose, saveNote, noteToEdit }) => {
    ...
    const validateForm = () => {
        ...
        if (!tag.trim()) {
            newErrors.tag = 'La etiqueta no puede estar vacía'
        }
        setErrors(newErrors)
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                <h2 className="text-2xl font-bold mb-4">{noteToEdit ? 'Editar Nota' : 'Agregar Nota'}</h2>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end">
                        ...
                        >
                            {noteToEdit ? 'Guardar Cambios' : 'Guardar'}
                        </button>
                    </div>
                </form>
            </div>
