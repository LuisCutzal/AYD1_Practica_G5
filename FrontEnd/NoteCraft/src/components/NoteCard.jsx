...
function NoteCard({id, title, description, tag, onDelete, onEdit, onPin, isPinned, onShare, isArchived, onArchive}) {
  const handleDelete = () => {
  ...
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white relative">
      ...
      <div className="absolute top-2 right-2 flex space-x-2">
        ...
        <button onClick={onArchive} className="text-gray-500 hover:text-gray-600">
          {isArchived ? '📤 Desarchivar' : '🗂️ Archivar'}
        </button>
      </div>
    </div>
  )
